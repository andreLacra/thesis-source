...
# class Video(object):
#     def __init__(self):
#         # Change the index in VideoCapture to match the index of your external webcam
#         self.video = cv2.VideoCapture(0)  # Change the index to match your external webcam
...
#         while np.all(frame == 0):
#             print("Frame is black. Reopening video capture...")
#             self.video.release()
#             self.video = cv2.VideoCapture(0)  # Reopen the video capture
#             ret, frame = self.video.read()  # Read a new frame
#             if not ret:
#                 print("Failed to read frame from the video capture after reopening")
#                 return None
